...
class CIFS(nn.Module):
    ...
    def _get_mask_with_graph(self, feat, y=None):
        ...
        # 主要就是对下面的语句进行修改，尤其是adapive_avg_pool2d()的使用，最后的mask的形状应当是(N, 1, H, W)
...
class Cubic_Probe_Block(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, size, stride=1, mode='linear'):
        super(Cubic_Probe_Block, self).__init__()
...
class Cubic_Probe_Resnet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        super(Cubic_Probe_Resnet, self).__init__()
        self.in_planes = 64
        ...
        self.layer4 = self._make_layer_(Cubic_Probe_Block, 512, num_blocks[3], stride=2, size=4*4)
...
def cubic_probe_resnet18(**kwargs):
    return Cubic_Probe_Resnet(BasicBlock, [2, 2, 2, 2], **kwargs)
# ------------