...
class CIFS(nn.Module):
    def __init__(self, n_feat=512, n_cls=10, mode='linear'):
        ...
        if mode == 'linear':
            self.Probe = nn.Sequential(
                *[Global_Avg_Flatten(), nn.Linear(n_feat, n_cls)])
        else:
            self.Probe = nn.Sequential(
                *[nlBlock(n_feat, 128), nn.Linear(128, n_cls)])
    ...
    def _get_mask_with_graph(self, feat, y=None):
        ...
        mask = torch.autograd.grad(max_logit, feat, create_graph=True, retain_graph=True)[0]
        mask = F.adaptive_avg_pool2d(
            mask, (1, 1)) * mask.size(2) * mask.size(3)
...
class Channel_Probe_Block(nn.Module):
    expansion = 1

    def __init__(self, in_planes, planes, stride=1, mode='linear'):
        super(Channel_Probe_Block, self).__init__()
        self.conv1 = nn.Conv2d(
            in_planes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(planes)
    ...
    def forward(self, x, label=None):
        ...
        # Channel_Purifier
        fc_out = self.fc(torch.mean(
            out.view(out.shape[0], out.shape[1], -1), dim=-1))
        if self.training:
            N, C, _, _ = out.shape
        ...
        # CChannel_ResNet中forward的最后几步，和CIFS_ResNet基本一致，所以可以直接套用CIFS中Probe的结构 ————2023.06.26（明天回来再看）

        masked_out, pred_raw = self.Probe(out, label)
        return masked_out, pred_raw, out, fc_out  # 此处的顺序待定


class Channel_Probe_Resnet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        super(Channel_Probe_Resnet, self).__init__()
        self.in_planes = 64
        ...
        self.layer4 = self._make_layer_(
            Channel_Probe_Block, 512, num_blocks[3], stride=2)
...
def channel_resnet18_probe(**kwargs):
    return Channel_Probe_Resnet(BasicBlock, [2, 2, 2, 2], **kwargs)
