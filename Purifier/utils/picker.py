...
sys.path.append('/home/jinyulin/Purifier/Models/')
...
from Purifier.Models.ResNet.ChannelResNet_probe import channel_resnet18_probe
from Purifier.Models.ResNet.CubicResNet_probe import cubic_probe_resnet18
from Purifier.Models.ResNet.FeatureResNet_probe import feature_probe_resnet18
...
def model_picker(config:dict)->nn.Module:
    ...
    elif config['Global']['model'] == 'Channel_ResNet18_probe':
        if config['Global']['dataset'] == 'GTSRB':
            model = channel_resnet18_probe(num_classes=43).to(config['Global']['device'])
        else:
            model = channel_resnet18_probe(num_classes=10).to(config['Global']['device'])

    elif config['Global']['model'] == 'Cubic_ResNet18_probe':
        if config['Global']['dataset'] == 'GTSRB':
            model = cubic_probe_resnet18(num_classes=43).to(config['Global']['device'])
        else:
            model = cubic_probe_resnet18(num_classes=10).to(config['Global']['device'])
    elif config['Global']['model'] == 'Feature_ResNet18_probe':
        if config['Global']['dataset'] == 'GTSRB':
            model = feature_probe_resnet18(num_classes=43).to(config['Global']['device'])
        else:
            model = feature_probe_resnet18(num_classes=10).to(config['Global']['device'])
